module mod_adsr
    ...
    type, public :: adsr_t
        ...
        integer       :: state         ! Which stage are we in?
        real(kind=dp) :: current_level ! Current volume (0.0 to 1.0)
    contains
...
contains
    ...
    function init_adsr(a, d, s, r, rate) result(this)
        ...
        this%state         = IDLE
        this%current_level = 0.0_dp
    end function init_adsr

    ! Trigger the envelope start (Key Press)
    subroutine note_on(this)
        class(adsr_t), intent(inout) :: this
        this%state = ATTACK
    end subroutine note_on

    ! Trigger the envelope end (Key Release)
    ...
    function get_next_level(this) result(level)
        ...
        ! STATE 1: ATTACK (Rise to 1.0)
        case (ATTACK)
            ! Rate: Distance (1.0) / Steps (Time * Rate)
            step = 1.0_dp / (this%attack_time * this%sample_rate)
            this%current_level = this%current_level + step
            
            ! Transition Check
            if (this%current_level >= 1.0_dp) then
                this%current_level = 1.0_dp
                this%state = DECAY
            end if

        
        ! STATE 2: DECAY (Fall to Sustain Level)
        case (DECAY)
            ...
            ! Transition Check
            if (this%current_level <= this%sustain_level) then
                this%current_level = this%sustain_level
                this%state = SUSTAIN
            end if

        
        ! STATE 3: SUSTAIN (Hold steady)
        case (SUSTAIN)
            this%current_level = this%sustain_level

        
        ! STATE 4: RELEASE (Fall to 0.0)
        case (RELEASE)
            ! We always use the full scale rate for consistent slope
            step = 1.0_dp / (this%release_time * this%sample_rate)
            this%current_level = this%current_level - step
            
            ! Transition Check
            if (this%current_level <= 0.0_dp) then
                this%current_level = 0.0_dp
                this%state = IDLE
            end if
